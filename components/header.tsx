...
export default function Header() {
  ...
  const navLinks = [
    { href: '/about-us', label: 'Experts', id: 'team' },
            ...
            >
              {navLinks.map((link) => (
                <motion.div key={link.href} variants={fadeIn}>
                  <Link
                    href={link.href}
                    className={`text-[14px] uppercase tracking-[0.25em] font-medium transition-all duration-300 relative group ${activeSection === link.id ? 'text-accent' : 'text-foreground/70 hover:text-accent'}`}
                  >
                    {link.label}
